...
class PinViewTests(TestCase):
    ...
    def test_create_pin(self):
        data = {
            ...
            'image': 'path/to/image.jpg',  # Use um arquivo real ou mock se necessário
        }
    ...
    def test_list_pins(self):
        Pin.objects.create(title='Pin  1', description='Description  1', user=self.user)
        Pin.objects.create(title='Pin  2', description='Description  2', user=self.user)
    ...
    def test_update_pin(self):
        pin = Pin.objects.create(title='Old Title', description='Old Description', user=self.user)
        new_data = {
            'title': 'New Title',
            'description': 'New Description',
        }
        response = self.client.put(reverse('pin-detail', args=[pin.id]), new_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        pin.refresh_from_db()
    ...
    def test_delete_pin(self):
        pin = Pin.objects.create(title='To Delete', description='Delete Me', user=self.user)
